<template>
  <div class="demo">
    ...
    <h3>文件单选OSS</h3>
    <input type="file" ref="file" @change="fileUploadOss($event)">
    <button @click="submitOss">提交</button>
    <div class="img">
      <img :src="imgOss" alt="">
    </div>
...
<script>
import { upload, upload1, getImgList,uploadOss } from "@/api/upload";
export default {
  name: "demo",
  components: {},
  data() {
  ...
  methods: {
    ...
    fileUploadOss(event) {
      //this.file = this.$refs.file.files[0]; //获取文件
      this.fileOss = event.target.files[0];
      console.log(this.file);
      ...
      //创建图片文件的url，http格式的url路径
      this.imgOss = windowURL.createObjectURL(this.fileOss);
      //console.log(this.img);
    },
    submitOss() {
      uploadOss({ fileOss: this.fileOss }).then(res => {
